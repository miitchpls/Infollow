...
export const FollowersController = async (req: Request, res: Response) => {
  ...
  Bluebird.try(async () => {
    ...
    const followersFeed = await client.feed.accountFollowers(userId);

    // Load the state of the feed if present.
    if (req.body.feed) {
      followersFeed.deserialize(JSON.stringify(req.body.feed));
    }

    // Load user followers. Feeds are auto paginated.
    const followers =
      !req.body.feed || followersFeed.isMoreAvailable()
        ? await followersFeed.items()
        : {};

    // Return user followers information.
    res.status(HttpCode.OK).json({ feed: followersFeed, followers });
  })
